...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--cases", action="store", default="1", help="run testcases for the targeted CWEs")
    ...
    parser.add_argument("-t", "--run-timeout", action="store", default="1", type=float, help="specify the default test run timeout in seconds (type: float, default: .01)")
        ...
        if match != None:
            ...
            if (parsed_CWE in args.CWEs) or args.all:
                ...
                if args.run:
                    ...
                    run(parsed_CWE, args.output_dir, str(args.run_timeout) + "s", args.cases)
    sys.exit(0)
